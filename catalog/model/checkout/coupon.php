...
class ModelCheckoutCoupon extends Model {
	public function getCoupon($code, $noverify=0) {
		...
		$coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE LCASE(code) = '" . $this->db->escape(strtolower($code)) . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1'");
